...
import google.maps.*;
import model.*;
import model.wraper.BookMarkMonument;
import model.wraper.BookMarkPhoto;
import model.wraper.BookMarkRestaurant;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (Monument monument : db.selectMonuments()) {
			map.add(new BookMarkMonument(monument));
		}

		for (Photo photo : db.selectPhotos()) {
			map.add(new BookMarkPhoto(photo));
		}

		for (Restaurant restaurant : db.selectRestaurants()) {
			map.add(new BookMarkRestaurant(restaurant));
		}
